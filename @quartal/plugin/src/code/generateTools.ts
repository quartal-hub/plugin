...
import type { PluginRuntimeArtifacts, WidgetCatalogEntry } from "../model/index.ts";
 ...
 * - artifacts.ts (importable build-time snapshot: the JSON artifacts + manifest, README,
 *   qrtl.config runtime options and widget entries — see {@link PluginRuntimeArtifacts})
 *
...
export async function generateTools(options?: GenerateToolsOptions): Promise<void> {
  ...
  const readme = await Helpers.readIfExists(join(cwd, "README.md"));
  ...
  if (options?.emitRegistry ?? true) {
    // The build-time snapshot of everything else the runtime would read from disk: manifest,
    // README, qrtl.config runtime options and the resolved widget entries. The JSON artifacts are
    // imported (not inlined) so the files above stay the single source of truth.
    const widgetResources = toWidgetEntries(
      await discoverWidgets(join(cwd, WIDGET_PAGES_DIR), qrtlConfig?.widgets),
    );
    const source = buildArtifactsModuleSource({
      manifest,
      readme,
...
/**
 * Renders the generated `artifacts.ts` module: imports of the sibling JSON artifacts plus inlined
 * literals for the build-time-resolved values (see {@link PluginRuntimeArtifacts}). `@ts-nocheck`
 * keeps the module out of consumer typechecks (JSON imports would otherwise require
 * `resolveJsonModule` in every plugin's tsconfig); Vite bundles the JSON imports into the server
 * output, which is what makes a bundled deployment self-contained.
...
export function buildArtifactsModuleSource(
  ...
  const optional = (key: "readme" | "mcp") =>
    values[key] === undefined ? [] : [`  ${key}: ${literal(values[key])},`];
  return [
    `// AUTO-GENERATED by @quartal/plugin — build-time snapshot of the plugin's runtime metadata.`,
    `// The generated middleware passes this to getAnonApp/getAuthApp so a bundled deployment`,
    `// (serverless function, Worker) serves the plugin without reading these files from disk.`,
    `// @ts-nocheck`,
    `import tools from "./tools.json";`,
    `import openApi from "./open-api.json";`,
    `import types from "./types.json";`,
    `import contents from "./contents.json";`,
    `import mcpTools from "./mcp-tools.json";`,
    `import mcpPrompts from "./mcp-prompts.json";`,
    ``,
    ...
    `  mcpTools,`,
    `  mcpPrompts,`,
    `  manifest: ${literal(values.manifest)},`,
    ...optional("readme"),
